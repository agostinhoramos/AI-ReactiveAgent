...
var MOVE = {
    ...
    POS: function (c) {
        let POS = null;
        if(
            ( GEO.LIMIT.W(c) || GEO.LIMIT.NW(c) ) //X4
            &&
            ( !GEO.LIMIT.N(c) && !GEO.LIMIT.NE(c) ) //X1
        ){
            POS = GEO.N(c); // N
            this.LAST_DIR = 1;
        }else
        if(
            ( GEO.LIMIT.N(c) || GEO.LIMIT.NE(c) ) //X1
            &&
            ( !GEO.LIMIT.E(c) && !GEO.LIMIT.SE(c) ) //X2
        ){
            POS = GEO.E(c); // E
            this.LAST_DIR = 2;
        }else
        if(
            ( GEO.LIMIT.E(c) || GEO.LIMIT.SE(c) ) //X2
            &&
            ( !GEO.LIMIT.S(c) && !GEO.LIMIT.SW(c) ) //X3
        ){
            POS = GEO.S(c); // S
            this.LAST_DIR = 3;
        }else
...
var Thread = {
    ...
    next: function (callback) {
        ...
        if ( // CONCLUSION
            ...
            if (!$.agent.conclude) {
                output.result.write('O agente entra em loop infinito!');
                $.agent.conclude = !0;
            }
        }
        ...
        Thread.id = setInterval(() => {
            Thread.next((e) => {
                if (MOVE.LAST_DIR > 0) {
                    ...
                    output.sensors8([
                        [
                            (GEO.LIMIT.N(e)||GEO.LIMIT.NE(e)) ? 0 : 1,
                            (GEO.LIMIT.E(e)||GEO.LIMIT.SE(e)) ? 0 : 1,
                            (GEO.LIMIT.S(e)||GEO.LIMIT.SW(e)) ? 0 : 1,
                            (GEO.LIMIT.W(e)||GEO.LIMIT.NW(e)) ? 0 : 1
                        ],
                        [
                            e,
                            MOVE.LAST_DIR
                    ...
                    output.cell(e, ['NILL']);
                    if( !$.agent.conclude ){
                        output.result.write('O agente não sai de lugar!');
                    }
                    $.agent.conclude = !0;
                }
            });
...
var output = {
    ...
    step_role: function (nbr) {
        $(".demo-step .athm p").attr('active', null);
        $(".demo-step .athm p").eq(nbr - 1).attr('active', !0);
    }
};
...
function astep(e, d) {
    ...
    output.cell( GEO.N(a), d[0][0] ? 'tmp_yes' : 'tmp_no' );
    output.cell( GEO.NE(a), d[0][0] ? 'tmp_yes' : 'tmp_no' );

    output.cell( GEO.E(a), d[0][1] ? 'tmp_yes' : 'tmp_no' );
    output.cell( GEO.SE(a), d[0][1] ? 'tmp_yes' : 'tmp_no' );

    output.cell( GEO.S(a), d[0][2] ? 'tmp_yes' : 'tmp_no' );
    output.cell( GEO.SW(a), d[0][2] ? 'tmp_yes' : 'tmp_no' );

    output.cell( GEO.W(a), d[0][3] ? 'tmp_yes' : 'tmp_no');
    output.cell( GEO.NW(a), d[0][3] ? 'tmp_yes' : 'tmp_no');
